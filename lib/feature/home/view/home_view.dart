...
import 'package:alejandroloi/feature/project/view/project_detail.dart';
...
class _HomeScreenViewState extends State<HomeScreenView> {
  ...
  void initState() {
    ...
    _investPage = PageController(viewportFraction: 0.78);
    _investPage.addListener(() {
      final p = _investPage.page ?? 0.0;
      if (mounted) setState(() => _investPagePos = p);
    });
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (investsList != null) {
        for (final raw in investsList.take(8)) {
          final m = Map<String, dynamic>.from(raw as Map);
      ...
      if (projectsList != null) {
        for (final raw in projectsList.take(5)) {
          final m = Map<String, dynamic>.from(raw as Map);
          _projectIds.add((m['id'] ?? m['_id'] ?? '').toString());
          _projects.add(
            _ProjectMini(
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
              ...
              SizedBox(
                ...
                child: PageView.builder(
                  ...
                  itemBuilder: (_, index) {
                    ...
                    final delta = (_investPagePos - index).abs();
              ...
              if ((_invests.isNotEmpty ? _invests.length : 3) > 1) ...[
                const SizedBox(height: 10),
                Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: List.generate(
                    _invests.isNotEmpty ? _invests.length : 3,
                        (i) {
                      final active = (i - _investPagePos).abs() < 0.5;
